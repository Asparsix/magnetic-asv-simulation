# NIOT Magnetic ASV Simulation — Version 2

Version 2 is an independent ROS 2 Jazzy + Gazebo Harmonic workspace. It is
stored separately from the repository's original implementation; no legacy
source files are replaced or required at runtime.

Key additions in this version:
- differential-thrust Gazebo boat with LOS/PID path following;
- synthetic 50 nT dipole anomaly over a 45,000 nT background with 3 nT noise;
- calibration, 10 m Bayesian belief grid, and weighted-centroid estimate;
- lawnmower, mutual-information, expanding-ring, and verification mission modes;
- continuous dipole least-squares refinement, demonstrated at approximately
  1.3 m error with spatially diverse simulation samples;
- normal and accelerated physics modes plus rosbag record/replay;
- live trajectory, magnetic readings, centroid, and dipole-fix visualization.
...
```bash
git clone https://github.com/Asparsix/magnetic-asv-simulation.git
cd magnetic-asv-simulation/version_2
