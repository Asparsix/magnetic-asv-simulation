...
=\frac{3(\mathbf{m}\cdot\hat{\mathbf{r}})\hat{\mathbf{r}}-\mathbf{m}}{r^{3}+s^{3}},
\qquad
\mathbf{r}=\mathbf{p}-\mathbf{t},\ \mathbf{t}=(t_x,t_y,t_z).
\]

Total-field anomaly (full, not first-order):

\[
\Delta F = \bigl|\mathbf{F}+\mathbf{B}_d\bigr| - |\mathbf{F}|.
\]
...
Mission calibration: \(s=11.5\,\mathrm{m}\) (via `calibrate_soft_m_for_mission()` in `dipole.py`) with vertical \(\mathbf{m}\) auto-sized so overhead \(|\Delta F|\approx 50\,\mathrm{nT}\) at \(t_z=-1\,\mathrm{m}\). At 10 m offset \(|\Delta F|\approx 14\,\mathrm{nT}\) (written 7–10 m sensing band); at 5 m (worst 10 m lawnmower offset) \(|\Delta F|\gtrsim 19\,\mathrm{nT}\) so HIT \(\ge 15\,\mathrm{nT}\) holds. Set `dipole_mx/my/mz` explicitly to plant a tilted moment. Fallback: `dipole_model: scalar_soft` restores scalar \(A/(r^3+s^3)\) on \(B_z\) only.
